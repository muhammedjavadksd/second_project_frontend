                                        ...
                                        item && item.length ? (
                                            (
                                                ...
                                                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                                    <TableHead head={['ID', 'Title', 'Category', 'Priority', 'Status', 'Updated Date', 'Action']} />
                                                    {
                                                        item.map((ticket: ProfileTicket, index) => {
                                                            return <TableBody key={ticket.ticket_id} data={[ticket.ticket_id, ticket.title, ticket.category, ticket.priority, ticket.status, formatDateToMonthNameAndDate(ticket.updated_at), <Link key={ticket.ticket_id} className='p-3 px-5 rounded-md text-white bg-blue-600' href={`${ticket.ticket_id}`}>View</Link>]} />
                                                        })
                                                    }
                                                </table>
                                            )
