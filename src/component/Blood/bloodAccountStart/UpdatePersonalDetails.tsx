    ...
    return (
        <LoadingComponent closeOnClick={false} isLoading={isLoading} paddingNeed={false}>
            <div className='bg-white  rounded-t p-5 rounded-b min-h-10 min-w-96 '>
                ...
                <Formik initialValues={initialData && updateBloodDonorPersonalDetailsValues(initialData)} enableReinitialize validationSchema={updateDonorPersonDetailsValidation} onSubmit={(val) => { OnDonorPersonDataEditSubmit(val, successCB, errorCb) }}>
                    {({ dirty }) => (
                        <Form>
                            <div className='mb-5'>
