...
let personalDetailsValidation = yup.object().shape({
    raiser_name: yup.string().trim().typeError("Please enter valid name").required("Name is required"),
    ...
    benificiary_relation: yup.string().trim().typeError("Please seleoct valid realtion").oneOf(const_data.RELATIONSHIP, "Please select valid fund raiser").required("Relation is required"),
